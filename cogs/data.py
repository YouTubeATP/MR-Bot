...
class Data(commands.Cog):
    ...
    async def data(self, ctx, stationCode=None, lang=None):
        ...
        linee = ""
        if lang == "eng":
            embed=discord.Embed(title="MR", description=f"{name} {stationType}")
            for line in lines:
                linee += lineNames[line] + ", "
            embed.add_field(name="Passing Lines", value=linee[:-2], inline=True)
            embed.add_field(name="No. of exits", value=exits, inline=True)
        elif lang == "chi":
            embed=discord.Embed(title="MR", description=f"{name}{stationType}")
            for line in lines:
                linee += lineNames[line] + "、"
            embed.add_field(name="途經路綫", value=linee[:-1], inline=True)
            embed.add_field(name="出口數量", value=exits, inline=True)
